...
# Redundant channel next to colour so bars survive greyscale printing.
FRAMEWORK_HATCH = {
    "Dedupe":          "//",
    "Splink":          "\\\\",
    "Zingg":           "xx",
...
# The CCER datasets are presented as D1..D8 (source ids stay D2..D9).
DISPLAY_NAME = {f"D{i}": f"D{i - 1}" for i in range(2, 10)}


def disp(dataset):
    """Name a dataset is shown under."""
...
# ------------------------------------------------------- ranking heatmap ----
def plot_rank_heatmap(combined, metric, nice, out_path):
    """One row per framework, one column per dataset, cell = rank position.

    CCER (D1..D8) and DER (CORA, CDDB) sit in the same figure; the last column
    holds the mean rank over the CCER datasets.  Ranks are computed within each
    column over the frameworks that actually ran that dataset, so DER columns
    rank 4 frameworks and CCER columns rank 7.  Cells are shaded by relative
    position inside their own column (dark = best) so the two blocks stay
    comparable; the three frameworks that never ran DER get empty cells.
    """
    ccer_fw = [fw for fw in FRAMEWORK_ORDER if fw in set(combined["framework"])]
    ccer_M = rank_matrix(combined, metric, CCER_DATASETS, ccer_fw)
    ccer_ranks = ccer_M.rank(axis=1, ascending=False, method="average")
    avg = ccer_ranks.mean(axis=0)

    der_fw = [fw for fw in ccer_fw if fw in DER_FRAMEWORKS]
    der_M = rank_matrix(combined, metric, DER_DATASETS, der_fw)
    der_ranks = der_M.rank(axis=1, ascending=False, method="average")

    rows = avg.sort_values().index.tolist()          # best framework on top
    cols = ([(disp(d), ccer_ranks, len(ccer_fw)) for d in CCER_DATASETS] +
            [(d, der_ranks, len(der_fw)) for d in DER_DATASETS])

    n_r, n_c = len(rows), len(cols) + 1
    ranks = np.full((n_r, n_c), np.nan)              # raw rank (annotation)
    shade = np.full((n_r, n_c), np.nan)              # 0 = best .. 1 = worst
    for j, (label, rank_df, k_col) in enumerate(cols):
        src = CCER_DATASETS[j] if j < len(CCER_DATASETS) else DER_DATASETS[j - len(CCER_DATASETS)]
        for i, fw in enumerate(rows):
            if fw not in rank_df.columns or pd.isna(rank_df.loc[src, fw]):
                continue
            r = float(rank_df.loc[src, fw])
            ranks[i, j] = r
            shade[i, j] = (r - 1.0) / max(k_col - 1, 1)
    for i, fw in enumerate(rows):
        ranks[i, -1] = avg[fw]
        shade[i, -1] = (avg[fw] - 1.0) / max(len(ccer_fw) - 1, 1)

    col_labels = [c[0] for c in cols] + ["Avg rank\n(CCER)"]

    cmap = plt.get_cmap("Blues_r").copy()            # single hue, dark = best
    cmap.set_bad("#ececec")
    fig, ax = plt.subplots(figsize=(1.02 * n_c + 2.6, 0.56 * n_r + 2.4))
    ax.imshow(np.ma.masked_invalid(shade), cmap=cmap, vmin=-0.05, vmax=1.25,
              aspect="auto")
    ...
    for i in range(n_r):
        for j in range(n_c):
            if np.isnan(ranks[i, j]):
                ...
                continue
            v = ranks[i, j]
            txt = f"{v:.2f}" if j == n_c - 1 else (
                f"{v:.0f}" if float(v).is_integer() else f"{v:.1f}")
            ax.text(j, i, txt, ha="center", va="center",
                    fontsize=11.5 if j == n_c - 1 else 11,
                    fontweight="bold" if j == n_c - 1 else "normal",
                    color="white" if shade[i, j] < 0.45 else "#222222")
    ...
    # thin white gutters between cells + separators around the blocks
    ...
    for x in (len(CCER_DATASETS) - 0.5, n_c - 1.5):
        ax.axvline(x, color="#555555", linewidth=1.8)
    ax.text((len(CCER_DATASETS) - 1) / 2, -0.85, "CCER (clean-clean)",
            ha="center", va="center", fontsize=11, fontweight="bold")
    ax.text(len(CCER_DATASETS) + 0.5, -0.85, "DER (dirty)",
            ha="center", va="center", fontsize=11, fontweight="bold")
    ax.set_title(f"Ranking position per dataset — {nice}  (1 = best)",
                 fontsize=13, pad=34)
    ax.text(0.5, -0.14,
            "Ranks are within a column: 7 frameworks on CCER, 4 on DER "
            "(Magellan, LinkTransformer and RecordLinkage are two-source only "
            "and did not run DER, shown as —).\n"
            "Shading is the relative position inside each column (darker = "
            "better), so CCER and DER columns stay comparable.",
            transform=ax.transAxes, ha="center", va="top", fontsize=8.5,
            style="italic", color="#555555")
    fig.tight_layout()
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    return ccer_ranks, der_ranks, avg
...
def plot_scalability(combined, datasets, out_path):
    ...
    # CCER datasets are presented as D1..D8 (source ids D2..D9); naming only.
    ax.set_title("Scalability: runtime vs. dataset size (CCER, D1–D8)")
    ax.grid(True, which="both", linestyle="--", alpha=0.35)
    ax.set_axisbelow(True)
...
def main():
    ...
    # ============ ranking heatmap (CCER + DER in one figure) ============
    log("\n" + "=" * 70)
    log("RANKING HEATMAP  (rank per dataset, mean rank over CCER)")
    log("=" * 70)
    for metric, nice in METRICS:
        ccer_ranks, der_ranks, avg = plot_rank_heatmap(
            combined, metric, nice,
            os.path.join(STATS_DIR, f"rank_heatmap_{metric}.png"))
        ranks_out = ccer_ranks.copy()
        ranks_out.index = [disp(d) for d in ranks_out.index]
        ranks_out = pd.concat([ranks_out, der_ranks])
        ranks_out.loc["Avg rank (CCER)"] = avg
        ranks_out.round(2).to_csv(
            os.path.join(STATS_DIR, f"rank_heatmap_{metric}.csv"))
        log(f"  [ok] rank_heatmap_{metric}.png  ({nice})")
