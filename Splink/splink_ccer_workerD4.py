...
def main():
    ...
    acm_df = pd.read_csv(ACM_PATH, delimiter="%")
    dblp_df = pd.read_csv(DBLP_PATH, delimiter="%")
    train_df = pd.read_csv(TRAIN_PATH)
    valid_df = pd.read_csv(VALID_PATH)
    test_df = pd.read_csv(TEST_PATH)

    for df in [acm_df, dblp_df]:
    ...
    acm_df = acm_df.rename(columns={"id": "unique_id"})
    ...
    acm_s = acm_df[keep].copy()
    dblp_s = dblp_df[keep].copy()
    acm_s["source_dataset"] = "acm"
    ...
    total_possible = len(acm_s) * len(dblp_s)
    ...
    with open(m_patched, "w") as f:
        json.dump(mj, f)
    db_api2 = DuckDBAPI()
    linker = Linker([acm_s, dblp_s], m_patched, db_api2,
                    input_table_aliases=["acm", "dblp"])
