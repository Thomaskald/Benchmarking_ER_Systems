...
def main():
    ...
    linker.misc.save_model_to_json("/tmp/splink_m.json", overwrite=True)
    with open("/tmp/splink_m.json") as f:
        mj = json.load(f)
    mj["probability_two_random_records_match"] = rate
    with open("/tmp/splink_m_patched.json", "w") as f:
        json.dump(mj, f)
    db_api2 = DuckDBAPI()
    linker = Linker([abt_s, buy_s], "/tmp/splink_m_patched.json", db_api2,
                    input_table_aliases=["abt", "buy"])
