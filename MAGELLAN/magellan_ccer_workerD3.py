...
def build_candset(csv_path, amazon, gp):
    df = pd.read_csv(csv_path)
    df.insert(0, "_id", range(len(df)))
    em.set_key(df, "_id")
    em.set_ltable(df, amazon)
    em.set_rtable(df, gp)
...
def tfidf_cosine(candset, amazon, gp, tfidf=None):
    left_ids = candset["left_id"].values
    right_ids = candset["right_id"].values
    left_texts = amazon.loc[left_ids, "title"].values + " " + amazon.loc[left_ids, "description"].values
    right_texts = gp.loc[right_ids, "title"].values + " " + gp.loc[right_ids, "description"].values
    if tfidf is None:
...
def prep_matrix(candset, amazon, gp, match_f, col_means, fitted_tfidf, feat_cols_final):
    ...
    sims, _ = tfidf_cosine(candset, amazon, gp, tfidf=fitted_tfidf)
    H["tfidf_cosine"] = sims
...
def main():
    ...
    amazon = em.read_csv_metadata(AMAZON_PATH, key="id", sep="#")
    gp = em.read_csv_metadata(GP_PATH, key="id", sep="#")
    for df in [amazon, gp]:
    ...
    train_cand = build_candset(TRAIN_PATH, amazon, gp)
    valid_cand = build_candset(VALID_PATH, amazon, gp)
    test_cand = build_candset(TEST_PATH, amazon, gp)

    match_f = em.get_features_for_matching(amazon, gp, validate_inferred_attr_types=False)
    ...
    tfidf_tr, fitted = tfidf_cosine(train_cand, amazon, gp)
    ...
    X_valid, y_valid = prep_matrix(valid_cand, amazon, gp, match_f, col_means, fitted, feat_cols_final)
    X_test,  y_test  = prep_matrix(test_cand,  amazon, gp, match_f, col_means, fitted, feat_cols_final)
