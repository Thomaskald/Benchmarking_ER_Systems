...
def load_base_table(path):
    ...
    return pd.DataFrame(rows, columns=["id", "title", "authors", "venue", "year"])


def build_candset(csv_path, dblp, scholar):
    df = pd.read_csv(csv_path)
    df.insert(0, "_id", range(len(df)))
    em.set_key(df, "_id")
    em.set_ltable(df, dblp)
    em.set_rtable(df, scholar)
    em.set_fk_ltable(df, "left_id")
    em.set_fk_rtable(df, "right_id")
    return df


def tfidf_cosine(candset, dblp, scholar, tfidf=None):
    ...
    right_texts = (scholar.loc[right_ids, "title"].values + " " +
                   scholar.loc[right_ids, "authors"].values + " " +
                   scholar.loc[right_ids, "venue"].values)
    if tfidf is None:
...
def prep_matrix(candset, dblp, scholar, match_f, col_means, fitted_tfidf, feat_cols_final):
    ...
    sims, _ = tfidf_cosine(candset, dblp, scholar, tfidf=fitted_tfidf)
    H["tfidf_cosine"] = sims
...
def main():
    ...
    dblp = load_base_table(DBLP_PATH)
    scholar = load_base_table(SCHOLAR_PATH)
    em.set_key(dblp, "id")
    ...
    train_cand = build_candset(TRAIN_PATH, dblp, scholar)
    valid_cand = build_candset(VALID_PATH, dblp, scholar)
    test_cand = build_candset(TEST_PATH, dblp, scholar)

    match_f = em.get_features_for_matching(dblp, scholar, validate_inferred_attr_types=False)
    ...
    tfidf_tr, fitted = tfidf_cosine(train_cand, dblp, scholar)
    ...
    X_valid, y_valid = prep_matrix(valid_cand, dblp, scholar, match_f, col_means, fitted, feat_cols_final)
    X_test,  y_test  = prep_matrix(test_cand,  dblp, scholar, match_f, col_means, fitted, feat_cols_final)
