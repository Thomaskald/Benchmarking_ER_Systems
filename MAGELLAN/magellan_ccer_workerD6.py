...
def load_base_table(path):
    ...
    df = pd.DataFrame(rows, columns=["id", "title", "name", "episodeNumber", "seasonNumber"])
    return df


def build_candset(csv_path, imdb, tvdb):
...
def tfidf_cosine(candset, imdb, tvdb, tfidf=None):
...
def prep_matrix(candset, imdb, tvdb, match_f, col_means, fitted_tfidf, feat_cols_final):
    ...
    sims, _ = tfidf_cosine(candset, imdb, tvdb, tfidf=fitted_tfidf)
    H["tfidf_cosine"] = sims
...
def main():
    ...
    tvdb = load_base_table(TVDB_PATH)
    em.set_key(imdb, "id")
    em.set_key(tvdb, "id")
    ...
    train_cand = build_candset(TRAIN_PATH, imdb, tvdb)
    valid_cand = build_candset(VALID_PATH, imdb, tvdb)
    test_cand = build_candset(TEST_PATH, imdb, tvdb)

    match_f = em.get_features_for_matching(imdb, tvdb, validate_inferred_attr_types=False)
    ...
    tfidf_tr, fitted = tfidf_cosine(train_cand, imdb, tvdb)
    ...
    X_valid, y_valid = prep_matrix(valid_cand, imdb, tvdb, match_f, col_means, fitted, feat_cols_final)
    X_test,  y_test  = prep_matrix(test_cand,  imdb, tvdb, match_f, col_means, fitted, feat_cols_final)
