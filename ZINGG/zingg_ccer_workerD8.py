...
# --- D8 (Amazon <-> Walmart electronics) paths -- confirmed via inspection:
# the ID-aligned tables live in train_validation_test_sets/db8/. Row counts
# (tableA=2554, tableB=22074) match the well-known public Walmart-Amazon
# benchmark, where table A is the smaller Walmart catalog and table B is the
# larger Amazon catalog -- swap the two filenames below if that's backwards
# for your copy of the data.
AMAZON_PATH  = "/home/it2022025/er_scalability/train_validation_test_sets/db8/tableB.csv"
WALMART_PATH = "/home/it2022025/er_scalability/train_validation_test_sets/db8/tableA.csv"
...
def main():
    ...
    # D8 tables are comma-delimited. Use pandas' default (C) engine here --
    # engine="python" mis-tokenizes this dataset's doubled-quote encoding
    # (e.g. """Draper Infrared Remote Transmitter""") and throws spurious
    # "Expected N fields, saw M" ParserErrors. The C engine parses it cleanly.
    amazon_df = pd.read_csv(AMAZON_PATH, na_filter=False)
    walmart_df = pd.read_csv(WALMART_PATH, na_filter=False)
