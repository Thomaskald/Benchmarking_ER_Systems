...
def main():
    ...
    zingg_dir = f"/tmp/zingg_d3_cfg{cid}"
    ...
    model_id = f"d3_cfg{cid}"

    amazon_df = pd.read_csv(AMAZON_PATH, sep="#", engine="python", na_filter=False)
    gp_df = pd.read_csv(GP_PATH, sep="#", engine="python", na_filter=False)
    train_df = pd.read_csv(TRAIN_PATH)
    valid_df = pd.read_csv(VALID_PATH)
    test_df = pd.read_csv(TEST_PATH)
    amazon_idx = amazon_df.set_index("id")
    gp_idx = gp_df.set_index("id")
    ...
    for _, row in train_sample.iterrows():
        ...
        try:
            left, right = amazon_idx.loc[lid], gp_idx.loc[rid]
        except KeyError:
            continue
        rows.append({"id": str(lid), "title": str(left["title"]),
                     "manufacturer": str(left["manufacturer"]),
        ...
        rows.append({"id": str(rid), "title": str(right["title"]),
                     "manufacturer": str(right["manufacturer"]),
                     "description": str(right["description"]), "price": str(right["price"]),
                     "z_cluster": cluster_id, "z_isMatch": label, "z_zsource": "gp"})
        cluster_id += 1
    ...
    spark = SparkSession.builder.appName(f"ZinggD3cfg{cid}").getOrCreate()
    spark.sparkContext.setLogLevel("ERROR")

    # ---- Zingg arguments: TWO-SOURCE LINK (Clean-Clean ER, amazon <-> gp) ----
    args = Arguments()
    args.setFieldDefinition([
        FieldDefinition("id", "string", MatchType.DONT_USE),
        FieldDefinition("title", "string", MatchType.FUZZY),
        FieldDefinition("manufacturer", "string", MatchType.FUZZY),
    ...
    gp_pipe = CsvPipe("gp", GP_PATH); gp_pipe.addProperty("header", "true"); gp_pipe.addProperty("delimiter", "#")
    args.setData(amazon_pipe, gp_pipe)
    ...
    # ---- score pairs: within each output z_cluster, link amazon ids to gp ids ----
    score_map = {}
    for _, group in output_df.groupby("z_cluster"):
        ...
        gp_ids = [ids[i] for i, s in enumerate(sources) if s == "gp"]
