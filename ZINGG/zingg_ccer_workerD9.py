...
# --- D9 (DBLP <-> Scholar) paths -- confirmed via inspection: the
# ID-aligned tables live in train_validation_test_sets/db9/. Table A (2,516
# rows, clean academic entries) is DBLP; table B (61,353 rows, noisier
# scraped-looking entries) is Scholar.
DBLP_PATH    = "/home/it2022025/er_scalability/train_validation_test_sets/db9/tableA.csv"
SCHOLAR_PATH = "/home/it2022025/er_scalability/train_validation_test_sets/db9/tableB.csv"
...
def main():
    ...
    zingg_dir = f"/tmp/zingg_d9_cfg{cid}"
    ...
    model_id = f"d9_cfg{cid}"

    # D9 tables are comma-delimited. Use pandas' default (C) engine here --
    # engine="python" mis-tokenizes this dataset's doubled-quote encoding
    # (e.g. """Semantic Integration of...""") and throws spurious
    # "Expected N fields, saw M" ParserErrors. The C engine parses it cleanly.
    dblp_df = pd.read_csv(DBLP_PATH, na_filter=False)
    scholar_df = pd.read_csv(SCHOLAR_PATH, na_filter=False)
    train_df = pd.read_csv(TRAIN_PATH)
    valid_df = pd.read_csv(VALID_PATH)
    test_df = pd.read_csv(TEST_PATH)
    dblp_idx = dblp_df.set_index("id")
    scholar_idx = scholar_df.set_index("id")
    ...
    for _, row in train_sample.iterrows():
        ...
        try:
            left, right = dblp_idx.loc[lid], scholar_idx.loc[rid]
        except KeyError:
            continue
        rows.append({"id": str(lid), "title": str(left["title"]),
                     "authors": str(left["authors"]), "venue": str(left["venue"]),
                     "year": str(left["year"]),
                     "z_cluster": cluster_id, "z_isMatch": label, "z_zsource": "dblp"})
        rows.append({"id": str(rid), "title": str(right["title"]),
                     "authors": str(right["authors"]), "venue": str(right["venue"]),
                     "year": str(right["year"]),
                     "z_cluster": cluster_id, "z_isMatch": label, "z_zsource": "scholar"})
        cluster_id += 1
    ...
    spark = SparkSession.builder.appName(f"ZinggD9cfg{cid}").getOrCreate()
    spark.sparkContext.setLogLevel("ERROR")

    # ---- Zingg arguments: TWO-SOURCE LINK (Clean-Clean ER, dblp <-> scholar) ----
    args = Arguments()
    args.setFieldDefinition([
        ...
        FieldDefinition("authors", "string", MatchType.FUZZY),
        FieldDefinition("venue", "string", MatchType.FUZZY),
    ...
    dblp_pipe = CsvPipe("dblp", DBLP_PATH); dblp_pipe.addProperty("header", "true"); dblp_pipe.addProperty("delimiter", ",")
    scholar_pipe = CsvPipe("scholar", SCHOLAR_PATH); scholar_pipe.addProperty("header", "true"); scholar_pipe.addProperty("delimiter", ",")
    args.setData(dblp_pipe, scholar_pipe)
    ...
    # ---- score pairs: within each output z_cluster, link dblp ids to scholar ids ----
    score_map = {}
    for _, group in output_df.groupby("z_cluster"):
        sources = group["z_zsource"].values
        ids = group["id"].values
        scores = group["z_score"].astype(float).values
        dblp_ids = [ids[i] for i, s in enumerate(sources) if s == "dblp"]
        scholar_ids = [ids[i] for i, s in enumerate(sources) if s == "scholar"]
        score = float(scores.max())
        for a in dblp_ids:
            for b in scholar_ids:
