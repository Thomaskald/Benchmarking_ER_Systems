...
CDDB_PATH  = "/home/it2022025/er_scalability/datasets/CDDB/cddb.csv"
TRAIN_PATH = "/home/it2022025/er_scalability/train_validation_test_sets/cddb/train_set.csv"
VALID_PATH = "/home/it2022025/er_scalability/train_validation_test_sets/cddb/valid_set.csv"
TEST_PATH  = "/home/it2022025/er_scalability/train_validation_test_sets/cddb/test_set.csv"
...
def main():
    ...
    zingg_dir = f"/tmp/zingg_cddb_cfg{cid}"
    ...
    model_id = f"cddb_cfg{cid}"

    cddb_df = pd.read_csv(CDDB_PATH, engine="python", na_filter=False)   # CDDB comma-delimited
    train_df = pd.read_csv(TRAIN_PATH)
    valid_df = pd.read_csv(VALID_PATH)
    test_df = pd.read_csv(TEST_PATH)
    cddb_idx = cddb_df.set_index(ID_COL)
    ...
    for _, row in train_sample.iterrows():
        ...
        try:
            left, right = cddb_idx.loc[lid], cddb_idx.loc[rid]
        except KeyError:
            continue
        rows.append({ID_COL: str(lid),
                     "artist": str(left.get("artist", "")),
                     "title": str(left.get("title", "")),
                     "genre": str(left.get("genre", "")),
                     "category": str(left.get("category", "")),
                     "year": str(left.get("year", "")),
                     "z_cluster": cluster_id, "z_isMatch": label, "z_zsource": "left"})
        rows.append({ID_COL: str(rid),
                     "artist": str(right.get("artist", "")),
                     "title": str(right.get("title", "")),
                     "genre": str(right.get("genre", "")),
    ...
    spark = SparkSession.builder.appName(f"ZinggCDDBcfg{cid}").getOrCreate()
    ...
    args.setFieldDefinition([
        ...
        FieldDefinition("artist", "string", MatchType.FUZZY, MatchType.TEXT),
        FieldDefinition("title", "string", MatchType.FUZZY, MatchType.TEXT),
        FieldDefinition("genre", "string", MatchType.EXACT),
        FieldDefinition("category", "string", MatchType.EXACT),
        FieldDefinition("year", "string", MatchType.EXACT),
    ])
    ...
    data_pipe = CsvPipe("cddb", CDDB_PATH)
    ...
    # train + match (kept as in the validated CDDB laptop script -- no link phase)
