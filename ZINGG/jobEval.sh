#!/bin/bash
#SBATCH --job-name=zingg_eval
#SBATCH --output=zingg_eval_%j.out
#SBATCH --error=zingg_eval_%j.err
...
# B-cubed), all datasets, each re-run in its own subprocess (fresh Spark/JVM).
srun python3 -u zingg_bestconfig_eval.py

# Tip: watch live progress with:  tail -f zingg_eval_*.out
