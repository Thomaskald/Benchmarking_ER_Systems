...
def main():
    ...
    cora = pd.read_csv(CDDB_PATH, engine="python", na_filter=False)   # CDDB comma-delimited
    for col in ["artist", "title", "genre", "category", "year"]:
        cora[col] = cora[col].apply(clean_text)
    cora = cora.rename(columns={"id": "unique_id"})
    cora["unique_id"] = cora["unique_id"].astype(str)
    cora["first_artist"] = cora["artist"].apply(lambda x: x.split()[0] if x else None)
    ...
    settings = SettingsCreator(
        ...
        comparisons=[
            cl.JaroWinklerAtThresholds("artist", jw),
        ...
        blocking_rules_to_generate_predictions=[
            block_on("first_artist"),
            block_on("year"),
            block_on("substr(title, 1, 2)"),
            block_on("substr(title, 1, 3)"),
            block_on("substr(title, 1, 5)"),
            block_on("substr(artist, 1, 2)"),
            block_on("substr(artist, 1, 4)"),
            block_on("genre"),
            block_on("category"),
        ],
    ...
    m_path = f"/tmp/splink_cddb_m_cfg{cid}.json"
    m_patched = f"/tmp/splink_cddb_m_patched_cfg{cid}.json"
