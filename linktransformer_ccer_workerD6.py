...
# The D6 base tables use full ontology URIs as column headers. Map the fields
# used for matching to short names so the rest of the pipeline is clean.
COL_MAP = {
...
def build_paired_df(split_df, imdb_idx, tvdb_idx):
    df = split_df.copy()
    df["left_text"] = (imdb_idx.loc[df["left_id"].values, "title"].values + " " +
                       imdb_idx.loc[df["left_id"].values, "name"].values + " " +
                       imdb_idx.loc[df["left_id"].values, "episodeNumber"].values + " " +
                       imdb_idx.loc[df["left_id"].values, "seasonNumber"].values)
    df["right_text"] = (tvdb_idx.loc[df["right_id"].values, "title"].values + " " +
                        tvdb_idx.loc[df["right_id"].values, "name"].values + " " +
                        tvdb_idx.loc[df["right_id"].values, "episodeNumber"].values + " " +
                        tvdb_idx.loc[df["right_id"].values, "seasonNumber"].values)
...
def main():
    ...
    imdb = pd.read_csv(IMDB_PATH, delimiter="|")
    tvdb = pd.read_csv(TVDB_PATH, delimiter="|")
    imdb = imdb.rename(columns=COL_MAP)
    tvdb = tvdb.rename(columns=COL_MAP)
    for df in [imdb, tvdb]:
        ...
        df["seasonNumber"] = (pd.to_numeric(df["seasonNumber"], errors="coerce")
                              .fillna("").astype(str).str.replace(r"\.0$", "", regex=True))
    imdb_idx = imdb.set_index("id")
    tvdb_idx = tvdb.set_index("id")
    ...
    train_paired = build_paired_df(train_df, imdb_idx, tvdb_idx)
    valid_paired = build_paired_df(valid_df, imdb_idx, tvdb_idx)
    test_paired = build_paired_df(test_df, imdb_idx, tvdb_idx)
