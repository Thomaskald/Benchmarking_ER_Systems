...
def main():
    ...
    zingg_dir = f"/tmp/zingg_scale10k_cfg{cid}"
    ...
    FEBRL_COLS = ["given_name", "surname", "address_1", "suburb", "postcode", "state", "date_of_birth", "soc_sec_id", "phone_number"]
    rows = []
    cluster_id = 0
    ...
    spark = SparkSession.builder.appName(f"ZinggScale10Kcfg{cid}").getOrCreate()
    spark.sparkContext.setLogLevel("ERROR")
    spark.sparkContext.setCheckpointDir(zingg_dir)

    # ---- Zingg arguments: SINGLE-SOURCE dedup (FEBRL field MatchTypes) ----
    args = Arguments()
    ...
    data_pipe = CsvPipe("febrl10k", PROFILES_PATH)
    ...
    # train + match (single-source dedup; no link -- consistent with DER)
