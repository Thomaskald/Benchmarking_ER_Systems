...
ID_COL = "id"
# FEBRL person-record fields used for embeddings (from the scalability script)
BLOCK_ATTRS = ["given_name", "surname", "address_1", "suburb", "postcode"]
...
def score_split(split_df, predicted_pairs):
    # Score over the split-file labeled pairs (matches every other tool/worker).
    y_true, y_pred = [], []
...
def main():
    ...
    # Synthetic data has no gt.csv; pyJedAI runs fine without ground_truth,
    # and we score externally via the split labels (consistent with other tools).
    data = Data(dataset_1=df, id_column_name_1=ID_COL)
    ...
    w = EmbeddingsNNWorkFlow(
        ...
        name=f"FEBRL10K-cfg{cid}",
    )
    w.run(data, verbose=False)

    entity_ids = df[ID_COL].tolist()
