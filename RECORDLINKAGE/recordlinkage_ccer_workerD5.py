...
def main():
    ...
    compare.string("genre_list", "genre_list", method="cosine", label="genre_cosine")
    compare.exact("episodeNumber", "episodeNumber", label="episode_exact")
    compare.exact("seasonNumber", "seasonNumber", label="season_exact")
    features_train = compare.compute(train_index, imdb, tmdb)
    features_valid = compare.compute(valid_index, imdb, tmdb)
    features_test = compare.compute(test_index, imdb, tmdb)

    train_left_ids = pd.Index(train_df["left_id"].unique())
    train_right_ids = pd.Index(train_df["right_id"].unique())
    word_corpus = np.concatenate([get_texts(imdb, train_left_ids, ["title", "name"]),
                                  get_texts(tmdb, train_right_ids, ["title", "name"])])
    char_corpus = np.concatenate([get_texts(imdb, train_left_ids, ["title", "name"]),
                                  get_texts(tmdb, train_right_ids, ["title", "name"])])
    ...
    for pidx, fdf in [(train_index, features_train), (valid_index, features_valid), (test_index, features_test)]:
        fdf["tfidf_word"] = build_tfidf_sim(pidx, imdb, tmdb, word_vec, ["title", "name"], "tfidf_word")
        fdf["tfidf_char"] = build_tfidf_sim(pidx, imdb, tmdb, char_vec, ["title", "name"], "tfidf_char")
