...
def main():
    ...
    acm  = pd.read_csv(ACM_PATH, delimiter="%")
    for df in [dblp, acm]:
    ...
    acm["year"] = pd.to_numeric(acm["year"], errors="coerce")
    dblp.set_index("id", inplace=True)
    acm.set_index("id", inplace=True)
    ...
    features_train = compare.compute(train_index, dblp, acm)
    features_valid = compare.compute(valid_index, dblp, acm)
    features_test = compare.compute(test_index, dblp, acm)
    ...
    word_corpus = np.concatenate([get_texts(dblp, train_left_ids, ["title", "authors"]),
                                  get_texts(acm, train_right_ids, ["title", "authors"])])
    char_corpus = np.concatenate([get_texts(dblp, train_left_ids, ["title"]),
                                  get_texts(acm, train_right_ids, ["title"])])
    ...
    for pidx, fdf in [(train_index, features_train), (valid_index, features_valid), (test_index, features_test)]:
        fdf["tfidf_word"] = build_tfidf_sim(pidx, dblp, acm, word_vec, ["title", "authors"], "tfidf_word")
        fdf["tfidf_char"] = build_tfidf_sim(pidx, dblp, acm, char_vec, ["title"], "tfidf_char")
