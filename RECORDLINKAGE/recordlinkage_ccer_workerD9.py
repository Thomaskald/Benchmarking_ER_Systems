...
def main():
    ...
    dblp    = pd.read_csv(DBLP_PATH, delimiter=">")
    scholar = pd.read_csv(SCHOLAR_PATH, delimiter=">")
    for df in [dblp, scholar]:
        df["title"] = clean(df["title"].fillna(""))
        df["authors"] = clean(df["authors"].fillna(""))
        df["venue"] = clean(df["venue"].fillna(""))
    dblp["year"] = pd.to_numeric(dblp["year"], errors="coerce")
    scholar["year"] = pd.to_numeric(scholar["year"], errors="coerce")
    dblp.set_index("id", inplace=True)
    scholar.set_index("id", inplace=True)
    ...
    compare.string("authors", "authors", method="cosine", label="authors_cosine")
    compare.string("venue", "venue", method="cosine", label="venue_cosine")
    compare.exact("year", "year", label="year_exact")
    features_train = compare.compute(train_index, dblp, scholar)
    features_valid = compare.compute(valid_index, dblp, scholar)
    features_test = compare.compute(test_index, dblp, scholar)

    train_left_ids = pd.Index(train_df["left_id"].unique())
    train_right_ids = pd.Index(train_df["right_id"].unique())
    word_corpus = np.concatenate([get_texts(dblp, train_left_ids, ["title", "authors"]),
                                  get_texts(scholar, train_right_ids, ["title", "authors"])])
    char_corpus = np.concatenate([get_texts(dblp, train_left_ids, ["title"]),
                                  get_texts(scholar, train_right_ids, ["title"])])
    ...
    for pidx, fdf in [(train_index, features_train), (valid_index, features_valid), (test_index, features_test)]:
        fdf["tfidf_word"] = build_tfidf_sim(pidx, dblp, scholar, word_vec, ["title", "authors"], "tfidf_word")
        fdf["tfidf_char"] = build_tfidf_sim(pidx, dblp, scholar, char_vec, ["title"], "tfidf_char")
