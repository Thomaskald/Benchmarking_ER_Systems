...
def main():
    ...
    abt.set_index("id", inplace=True)
    buy.set_index("id", inplace=True)
    ...
    features_train = compare.compute(train_index, abt, buy)
    features_valid = compare.compute(valid_index, abt, buy)
    features_test = compare.compute(test_index, abt, buy)

    train_left_ids = pd.Index(train_df["left_id"].unique())
    train_right_ids = pd.Index(train_df["right_id"].unique())
    word_corpus = np.concatenate([get_texts(abt, train_left_ids, ["name", "description"]),
                                  get_texts(buy, train_right_ids, ["name", "description"])])
    char_corpus = np.concatenate([get_texts(abt, train_left_ids, ["name"]),
                                  get_texts(buy, train_right_ids, ["name"])])
    ...
    for pidx, fdf in [(train_index, features_train), (valid_index, features_valid), (test_index, features_test)]:
        fdf["tfidf_word"] = build_tfidf_sim(pidx, abt, buy, word_vec, ["name", "description"], "tfidf_word")
        fdf["tfidf_char"] = build_tfidf_sim(pidx, abt, buy, char_vec, ["name"], "tfidf_char")
        add_price_features(pidx, abt, buy, fdf)
