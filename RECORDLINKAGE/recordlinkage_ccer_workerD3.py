...
def add_price_features(pair_index, left_df, right_df, feat_df):
    lp = left_df.loc[pair_index.get_level_values(0), "price"].to_numpy(dtype=float)
    rp = right_df.loc[pair_index.get_level_values(1), "price"].to_numpy(dtype=float)
    both = ~(np.isnan(lp) | np.isnan(rp))
    lpf = np.where(np.isnan(lp), 0.0, lp)
    rpf = np.where(np.isnan(rp), 0.0, rp)
    ad = np.abs(lpf - rpf)
    mx = np.maximum(np.abs(lpf), np.abs(rpf)); mx[mx == 0] = 1.0
    feat_df["price_abs_diff"] = pd.array(ad, dtype="Float64")
    feat_df["price_rel_diff"] = pd.array(ad / mx, dtype="Float64")
    feat_df["price_both_known"] = pd.array(both.astype(float), dtype="Float64")
...
def main():
    ...
    gp     = pd.read_csv(GP_PATH, delimiter="#")
    for df in [amazon, gp]:
        df["title"] = clean(df["title"])
        df["description"] = clean(df["description"].fillna(""))
        df["manufacturer"] = clean(df["manufacturer"].fillna(""))
    amazon["price"] = pd.to_numeric(amazon["price"], errors="coerce")
    gp["price"] = pd.to_numeric(gp["price"], errors="coerce")
    amazon.set_index("id", inplace=True)
    gp.set_index("id", inplace=True)
    ...
    compare.string("description", "description", method="cosine", label="description_cosine")
    compare.string("manufacturer", "manufacturer", method="cosine", label="manufacturer_cosine")
    features_train = compare.compute(train_index, amazon, gp)
    features_valid = compare.compute(valid_index, amazon, gp)
    features_test = compare.compute(test_index, amazon, gp)

    train_left_ids = pd.Index(train_df["left_id"].unique())
    train_right_ids = pd.Index(train_df["right_id"].unique())
    word_corpus = np.concatenate([get_texts(amazon, train_left_ids, ["title", "description"]),
                                  get_texts(gp, train_right_ids, ["title", "description"])])
    char_corpus = np.concatenate([get_texts(amazon, train_left_ids, ["title"]),
                                  get_texts(gp, train_right_ids, ["title"])])
    ...
    for pidx, fdf in [(train_index, features_train), (valid_index, features_valid), (test_index, features_test)]:
        fdf["tfidf_word"] = build_tfidf_sim(pidx, amazon, gp, word_vec, ["title", "description"], "tfidf_word")
        fdf["tfidf_char"] = build_tfidf_sim(pidx, amazon, gp, char_vec, ["title"], "tfidf_char")
        add_price_features(pidx, amazon, gp, fdf)
