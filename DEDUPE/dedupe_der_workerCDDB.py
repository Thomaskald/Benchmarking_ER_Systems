...
# CDDB OOM guards (from the laptop script): cap training samples + records
# used for prepare_training. 'tracks' column excluded (very long -> OOM).
MAX_MATCHES  = 2500
MAX_DISTINCT = 7500
SAMPLE_SIZE  = 2000
...
def load_cddb(path):
    ...
    with open(path, encoding="utf-8") as f:
        reader = csv.DictReader(f)   # CDDB is comma-delimited
        for row in reader:
            try:
                rid = int(row["id"])
            ...
            records[rid] = {
                ...
                # tracks excluded -- very long string causes OOM
            }
    return records
...
def main():
    ...
    # subsample records for prepare_training to avoid OOM; full dict used for scoring
    sampled_keys = rng.sample(list(records.keys()), min(SAMPLE_SIZE, len(records)))
    training_records = {k: records[k] for k in sampled_keys}
