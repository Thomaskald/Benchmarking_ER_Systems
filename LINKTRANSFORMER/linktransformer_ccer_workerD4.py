...
def build_paired_df(split_df, dblp_idx, acm_idx):
    df = split_df.copy()
    ...
    df["right_text"] = (acm_idx.loc[df["right_id"].values, "title"].values + " " +
                        acm_idx.loc[df["right_id"].values, "authors"].values + " " +
                        acm_idx.loc[df["right_id"].values, "venue"].values)
...
def main():
    ...
    acm["year"] = pd.to_numeric(acm["year"], errors="coerce").fillna(0).astype(int)
    dblp_idx = dblp.set_index("id")
    acm_idx = acm.set_index("id")
    ...
    train_paired = build_paired_df(train_df, dblp_idx, acm_idx)
    valid_paired = build_paired_df(valid_df, dblp_idx, acm_idx)
    test_paired = build_paired_df(test_df, dblp_idx, acm_idx)
