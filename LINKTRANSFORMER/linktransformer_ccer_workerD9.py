...
# NOTE: in D9 the split pairing is left_id -> dblp, right_id -> scholar.
def build_paired_df(split_df, dblp_idx, scholar_idx):
    df = split_df.copy()
    df["left_text"] = (dblp_idx.loc[df["left_id"].values, "title"].values + " " +
                       dblp_idx.loc[df["left_id"].values, "authors"].values + " " +
                       dblp_idx.loc[df["left_id"].values, "venue"].values)
    df["right_text"] = (scholar_idx.loc[df["right_id"].values, "title"].values + " " +
                        scholar_idx.loc[df["right_id"].values, "authors"].values + " " +
                        scholar_idx.loc[df["right_id"].values, "venue"].values)
...
def main():
    ...
    dblp["year"] = pd.to_numeric(dblp["year"], errors="coerce").fillna(0).astype(int)
    scholar["year"] = pd.to_numeric(scholar["year"], errors="coerce").fillna(0).astype(int)
    dblp_idx = dblp.set_index("id")
    scholar_idx = scholar.set_index("id")
    ...
    train_paired = build_paired_df(train_df, dblp_idx, scholar_idx)
    valid_paired = build_paired_df(valid_df, dblp_idx, scholar_idx)
    test_paired = build_paired_df(test_df, dblp_idx, scholar_idx)
