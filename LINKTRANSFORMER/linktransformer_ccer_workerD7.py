...
# The D7 base tables use full ontology URIs as column headers. Map the fields
...
def build_paired_df(split_df, tmdb_idx, tvdb_idx):
    df = split_df.copy()
    df["left_text"] = (tmdb_idx.loc[df["left_id"].values, "title"].values + " " +
                       tmdb_idx.loc[df["left_id"].values, "name"].values + " " +
                       tmdb_idx.loc[df["left_id"].values, "episodeNumber"].values + " " +
                       tmdb_idx.loc[df["left_id"].values, "seasonNumber"].values)
...
def main():
    ...
    tmdb = tmdb.rename(columns=COL_MAP)
    ...
    train_paired = build_paired_df(train_df, tmdb_idx, tvdb_idx)
    valid_paired = build_paired_df(valid_df, tmdb_idx, tvdb_idx)
    test_paired = build_paired_df(test_df, tmdb_idx, tvdb_idx)
