...
# The D5 base tables use full ontology URIs as column headers. Map the fields
...
def build_paired_df(split_df, imdb_idx, tmdb_idx):
    df = split_df.copy()
    df["left_text"] = (imdb_idx.loc[df["left_id"].values, "title"].values + " " +
                       imdb_idx.loc[df["left_id"].values, "name"].values + " " +
                       imdb_idx.loc[df["left_id"].values, "genre_list"].values)
    df["right_text"] = (tmdb_idx.loc[df["right_id"].values, "title"].values + " " +
                        tmdb_idx.loc[df["right_id"].values, "name"].values + " " +
                        tmdb_idx.loc[df["right_id"].values, "genre_list"].values)
...
def main():
    ...
    for df in [imdb, tmdb]:
        ...
        df["genre_list"] = df["genre_list"].str.lower().str.strip().fillna("")
    ...
    train_paired = build_paired_df(train_df, imdb_idx, tmdb_idx)
    valid_paired = build_paired_df(valid_df, imdb_idx, tmdb_idx)
    test_paired = build_paired_df(test_df, imdb_idx, tmdb_idx)
