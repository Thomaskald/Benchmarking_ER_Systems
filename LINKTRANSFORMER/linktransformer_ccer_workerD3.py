...
def build_paired_df(split_df, amazon_idx, gp_idx):
    df = split_df.copy()
    df["left_text"] = (amazon_idx.loc[df["left_id"].values, "title"].values + " " +
                       amazon_idx.loc[df["left_id"].values, "manufacturer"].values + " " +
                       amazon_idx.loc[df["left_id"].values, "description"].values)
    df["right_text"] = (gp_idx.loc[df["right_id"].values, "title"].values + " " +
                        gp_idx.loc[df["right_id"].values, "manufacturer"].values + " " +
                        gp_idx.loc[df["right_id"].values, "description"].values)
...
def main():
    ...
    train_paired = build_paired_df(train_df, amazon_idx, gp_idx)
    valid_paired = build_paired_df(valid_df, amazon_idx, gp_idx)
    test_paired = build_paired_df(test_df, amazon_idx, gp_idx)
