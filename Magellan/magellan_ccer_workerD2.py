...
def build_candset(csv_path, abt, buy):
    df = pd.read_csv(csv_path)
    df.insert(0, "_id", range(len(df)))
    em.set_key(df, "_id")
    em.set_ltable(df, abt)
    em.set_rtable(df, buy)
    em.set_fk_ltable(df, "left_id")
    em.set_fk_rtable(df, "right_id")
    return df


def tfidf_cosine(candset, abt, buy, tfidf=None):
    left_ids = candset["left_id"].values
    right_ids = candset["right_id"].values
    left_texts = abt.loc[left_ids, "name"].values + " " + abt.loc[left_ids, "description"].values
    right_texts = buy.loc[right_ids, "name"].values + " " + buy.loc[right_ids, "description"].values
    if tfidf is None:
...
def prep_matrix(candset, abt, buy, match_f, col_means, fitted_tfidf, feat_cols_final):
    ...
    sims, _ = tfidf_cosine(candset, abt, buy, tfidf=fitted_tfidf)
    H["tfidf_cosine"] = sims
...
def main():
    ...
    abt = em.read_csv_metadata(ABT_PATH, key="id", sep="|")
    buy = em.read_csv_metadata(BUY_PATH, key="id", sep="|")
    for df in [abt, buy]:
    ...
    train_cand = build_candset(TRAIN_PATH, abt, buy)
    valid_cand = build_candset(VALID_PATH, abt, buy)
    test_cand = build_candset(TEST_PATH, abt, buy)

    match_f = em.get_features_for_matching(abt, buy, validate_inferred_attr_types=False)
    ...
    tfidf_tr, fitted = tfidf_cosine(train_cand, abt, buy)
    ...
    X_valid, y_valid = prep_matrix(valid_cand, abt, buy, match_f, col_means, fitted, feat_cols_final)
    X_test,  y_test  = prep_matrix(test_cand,  abt, buy, match_f, col_means, fitted, feat_cols_final)
