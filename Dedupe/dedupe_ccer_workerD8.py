...
D1_PATH    = "/home/it2022025/er_scalability/datasets/D8/amazon.csv"    # left_id side
D2_PATH    = "/home/it2022025/er_scalability/datasets/D8/walmart.csv"   # right_id side
...
def load_csv(path):
    ...
    with open(path, encoding="utf-8") as f:
        reader = csv.DictReader(f, delimiter="|")     # D8 uses |
        for row in reader:
            try:
...
def main():
    ...
    fields = [
        ...
        dedupe.variables.String("modelno", has_missing=True),
        dedupe.variables.String("brand", has_missing=True),
        dedupe.variables.Price("price", has_missing=True),
        dedupe.variables.String("dimensions", has_missing=True),
    ]
    ...
    deduper.prepare_training(abt, buy, training_file=training_file, sample_size=5000)
    deduper.train(
