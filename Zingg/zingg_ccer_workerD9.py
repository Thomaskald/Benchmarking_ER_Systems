...
# --- D9 (DBLP <-> Scholar) paths, following the same layout convention as
# D2/D7/D8. NOTE: inferred by analogy — the reference D9 script you sent used
# /home/thomas/train_test_valid_datasets/db9/{tableA,tableB}.csv (table A ==
# DBLP, table B == Scholar). Double check these against your actual
# it2022025 layout.
...
def main():
    ...
    # D9 tables are comma-delimited (no sep="|" like D2's abt/buy).
    dblp_df = pd.read_csv(DBLP_PATH, engine="python", na_filter=False)
    scholar_df = pd.read_csv(SCHOLAR_PATH, engine="python", na_filter=False)
