...
Both metrics are computed on the fixed test set (uniform with the DEDUPE/Magellan
evaluators, so numbers are comparable across frameworks and always finish in time):
reads each dataset's best config (max test_f1) from results/pyjedai_<DS>_configs.csv,
reruns that one pipeline, then over the test pairs reports
  - pairwise P/R/F1 (a test pair counts as matched iff the model co-clustered it), and
...
# DER keeps gt_sep/gt_header: the workflow's evaluate step needs ground_truth
# (B-cubed itself uses the test-set labels, not these files).
SCADS_ATTRS = [
...
def testset_metrics(test_df, is_match, tag_left, tag_right, symmetric):
    """Pairwise + test-set B-cubed over the fixed test pairs (uniform with DEDUPE).

    is_match(na, nb) -> bool tells whether the model co-clustered the two records.
...
def eval_ccer(ds, cfg, params):
    ...
    # Cross-source (d1_id, d2_id) pairs the model put in the same cluster.
    predicted_pairs = set()
    for cl in clusters:
        ids = list(cl)
        ...
        for a in a_ids:
            for b in b_ids:
                predicted_pairs.add((norm_id(d1_ids[a]), norm_id(d2_ids[b - n1])))

    test_df = pd.read_csv(cfg["test"])
    m = testset_metrics(test_df, lambda na, nb: (na, nb) in predicted_pairs,
                        "A:", "B:", symmetric=False)
    return m
...
def eval_der(ds, cfg, params):
    ...
    # ground_truth is required by the workflow's internal evaluate step (same as the
    # worker); it is NOT used for B-cubed -- B-cubed uses the test-set labels.
    ...
    entity_ids = [norm_id(x) for x in df[id_col].tolist()]
    # entity id -> predicted cluster id; two records match iff same cluster.
    ent2cluster = {}
    for cid, cl in enumerate(w.clusters):
        for i in cl:
            ent2cluster[entity_ids[i]] = cid

    def is_match(na, nb):
        ca = ent2cluster.get(na)
        return ca is not None and ca == ent2cluster.get(nb)

    test_df = pd.read_csv(cfg["test"])
    m = testset_metrics(test_df, is_match, "", "", symmetric=True)
    return m
