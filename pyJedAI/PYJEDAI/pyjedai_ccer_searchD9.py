...
# Per supervisor: do NOT use WeightedEdgePruning; search only the cardinality
# comparison-cleaning (meta-blocking) algorithms.
COMPARISON_CLEANERS = ["CardinalityEdgePruning", "CardinalityNodePruning"]

# Weighting schemes valid per family (verified against pyjedai source):
#   edge-centric (CEP): standard schemes
#   node-centric (CNP): only the normalized / EJS family
WS_EDGE = ["CBS", "ECBS", "JS", "EJS", "X2", "COSINE", "DICE"]
WS_NODE = ["CN-CBS", "SN-CBS", "CNC", "SNC", "CND", "SND", "CNJ", "SNJ", "EJS"]

CLUSTERERS = ["UniqueMappingClustering", "ConnectedComponentsClustering"]

OTHER = {
    "ratio":      ("uniform", 0.5, 0.95),
    "vectorizer": ("choice", ["tfidf", "tf", "boolean"]),
    "tokenizer":  ("choice", ["char_tokenizer", "word_tokenizer"]),
    "qgram":      ("intchoice", [2, 3, 4, 5]),
}


def sample_config(rng):
    cc = rng.choice(COMPARISON_CLEANERS)
    if cc == "CardinalityNodePruning":
        ws = rng.choice(WS_NODE)
    else:
        ws = rng.choice(WS_EDGE)
    cfg = {"comparison_cleaner": cc, "weighting_scheme": ws,
           "clusterer": rng.choice(CLUSTERERS)}
    for name, spec in OTHER.items():
        kind = spec[0]
        if kind == "uniform":
            cfg[name] = round(rng.uniform(spec[1], spec[2]), 4)
        elif kind in ("choice", "intchoice"):
            cfg[name] = rng.choice(spec[1])
    cfg["metric"] = FIXED_METRIC
    return cfg
...
def run_one(cfg, config_id):
    ...
    status = "OOM" if ("MemoryError" in proc.stderr
                       or "Cannot allocate memory" in proc.stderr
                       or proc.returncode == 137) else "ERROR"
...
def write_csv(rows):
    import csv
    cols = ["config_id", "status", "ratio", "comparison_cleaner", "weighting_scheme",
    ...
    with open(CSV_PATH, "w", newline="") as f:
        ...
        for r in rows:
            p = r.get("params", {})
            tp = r.get("test_point", {}) or {}
            w.writerow({
                "config_id": r.get("config_id"), "status": r.get("status"),
                "ratio": p.get("ratio"),
                "comparison_cleaner": p.get("comparison_cleaner"),
                "weighting_scheme": p.get("weighting_scheme"),
                "vectorizer": p.get("vectorizer"), "metric": p.get("metric"),
                "tokenizer": p.get("tokenizer"), "qgram": p.get("qgram"),
                "clusterer": p.get("clusterer"),
...
def main():
    ...
    print(f"pyJedAI CCER random search | dataset={DATASET} | B={B} | "
    ...
    for i in range(1, B + 1):
        ...
        if status == "OK":
            ...
            if status == "ERROR":
                print("      stderr:", res.get("stderr_tail", "")[-300:], flush=True)
        rows.append(res)
        write_csv(rows)
