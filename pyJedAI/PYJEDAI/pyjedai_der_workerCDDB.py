...
GT_PATH    = "/home/it2022025/er_scalability/datasets/CDDB/gt.csv"
...
ID_COL = "id"
BLOCK_ATTRS = ["artist", "title", "year"]   # CDDB restricts embedding attributes
...
def main():
    ...
    cddb_df = pd.read_csv(CDDB_PATH, engine="python", na_filter=False).astype(str)
    ...
    w = EmbeddingsNNWorkFlow(
        ...
        name=f"CDDB-DER-cfg{cid}",
    )
    w.run(data, verbose=False)

    entity_ids = cddb_df[ID_COL].tolist()
